    ...
    for idx, thread in enumerate(threads):
        ...
        for participant in participants:
            ...
            if pk and pk != client.user_id:
                recipient_id = pk
                recipient_username = getattr(participant, "username", None) or str(pk)
                break
...
def _process_followups(
    ...
    _load_all_conversations_to_memory(client, user, max_age_days)
    ...
    for thread in threads:
        ...
        try:
            thread_messages = client.direct_messages(thread_id, amount=50)
            if thread_messages:
                outbound = [m for m in thread_messages if m.user_id == client.user_id]
                inbound = [m for m in thread_messages if m.user_id != client.user_id]
...
def _client_for(username: str):
    ...
    try:
        ...
        try:
            ...
            insta_available = _importlib_util.find_spec("instagrapi") is not None
            logger.warning(
                "Runtime Python para @%s: %s (instagrapi importable=%s)",
                username,
                _sys.executable,
                "si" if insta_available else "no",
            )
        except Exception:
            pass
        logger.warning(
    ...
    try:
        ...
        if account and account.get("proxy_url"):
            ...
            raise RuntimeError(f"El proxy de @{username} no respondia�: {exc}") from exc
    ...
    try:
        ...
        raise RuntimeError(f"No hay sesia�n para {username}.") from exc
    except Exception as exc:
    ...
    if not has_valid_session_settings(cl):
        mark_connected(username, False)
        raise RuntimeError(
            f"La sesia�n guardada para {username} no contiene credenciales activas. Inicia� sesia�n nuevamente."
        )
...
def _filter_valid_sessions(targets: list[str]) -> list[str]:
    ...
    for user in targets:
        if not has_session(user):
            needing_login.append((user, "sin sesia�n guardada"))
            continue
        if not _ensure_session(user):
            needing_login.append((user, "sesia�n expirada"))
            continue
    ...
    if needing_login:
        ...
        if remaining:
            print("\nLas siguientes cuentas necesitan volver a iniciar sesia�n:")
            for user, reason in remaining:
                print(f" - @{user}: {reason}")
            if ask("-aIniciar sesia�n ahora? (s/N): ").strip().lower() == "s":
                ...
                warn("Se omitieron las cuentas sin sesia�n va�lida.")
    return verified
...
def _process_inbox(
    ...
    inbox = _fetch_inbox_threads(client, amount=20)
    if not inbox:
    ...
    for idx, thread in enumerate(inbox, start=1):
        ...
        if not last:
            continue
        if getattr(last, "user_id", None) == client.user_id:
            continue
        ...
        if last_id and last_seen_id == str(last_id):
            continue
        
        recipient_username = _resolve_username(client, thread, last.user_id) or str(last.user_id)
        _record_message_received(user, thread_id, last_id, recipient_username)
        ...
        try:
            ...
            dm = client.direct_send(reply, [last.user_id])
            message_id = getattr(dm, "id", None) or ""
            ...
            if calendar_message:
                ...
                _record_message_sent(user, thread_id, calendar_message, None, recipient_username, is_followup=False)
                _sleep_between_replies_sync(delay_min, delay_max, label="reply_delay")
...
def _process_followups_extended(
    ...
    for thread in threads:
        ...
        if last_followup_float and now_ts - last_followup_float < _MIN_TIME_FOR_FOLLOWUP:
            _append_message_log(
                {
                    "action": "followup_skip",
                    ...
                    "thread_id": str(thread_id),
                    "lead": recipient_username or str(recipient_id),
                    "seconds_since_last_followup": int(now_ts - last_followup_float),
                }
            )
            continue
