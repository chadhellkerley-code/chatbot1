...
def ensure_logged_in(
    ...
    def _new_context(use_storage: bool) -> tuple[BrowserContext, Page]:
    ...
    if storage_state.exists():
        ctx, page = _new_context(use_storage=True)
        _load_home(page)
    ...
    ctx, page = _new_context(use_storage=False)
    _load_home(page)
    ...
    try:
        ...
        raise _raise_login_failure(page, username, profile_root_path, exc) from exc

    if login_ok and is_logged_in(page):
    ...
    raise _raise_login_failure(page, username, profile_root_path)


def _load_home(page: Page) -> None:
    ...
    try:
        ...
        page.wait_for_selector("a[href='/direct/inbox/'], nav[role='navigation'], textarea", timeout=10_000)
    except Exception:
...
def _raise_login_failure(
    ...
    screenshot_path = _capture_failure_screenshot(page, profile_root_path, username)
    errors = get_login_errors(page)
...
def _capture_failure_screenshot(page: Page, profile_root_path: Path, username: str) -> Optional[Path]:
