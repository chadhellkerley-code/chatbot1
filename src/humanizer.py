...
async def human_wait(min_s: float = 0.2, max_s: float = 1.0) -> None:
...
async def human_type(locator, text: str, clear_first: bool = True) -> None:
    """
...
async def human_click(target: Union[Page, Locator], selector: Optional[str] = None) -> None:
    """
    ...
    try:
        ...
        pass
    await human_wait(0.05, 0.4)
    await locator.click()
    await human_wait(0.1, 0.6)

async def human_scroll(page, amount_px: int = 1200, steps: int = 6) -> None:
    """Desplazamiento suave por la página."""
    step = max(1, int(amount_px / max(1, steps)))
    for _ in range(max(1, steps)):
        await page.mouse.wheel(0, step)
        await human_wait(0.2, 0.7)

async def human_mouse_trace(page, x: Optional[float] = None, y: Optional[float] = None, steps: int = 8) -> None:
    """
    ...
    try:
        if x is None or y is None:
            ...
            for dx, dy in [(4, 2), (-3, -1), (5, 1), (-2, 0)]:
                await pos.move(pos.position[0] + dx, pos.position[1] + dy, steps=random.randint(2, 6))
                await human_wait(0.02, 0.08)
        else:
            await page.mouse.move(x, y, steps=max(2, steps))
            await human_wait(0.05, 0.2)
    except Exception:
