...
def _client_for(username: str):
    ...
    try:
        ...
        raise RuntimeError(
            f"No hay sesión guardada para {username}. Iniciá sesión desde el menú."
        ) from exc
    except Exception as exc:
...
def _resolve_usertags(client, usernames: Sequence[str]):
    ...
    for username in usernames:
        try:
            ...
            continue
        tags.append(
            {"user": {"pk": user_id, "username": username}, "x": random.random(), "y": random.random()}
        )
    return tags or None
